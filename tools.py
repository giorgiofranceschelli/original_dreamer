...
class Adam(tf.Module):
    ...
    def __init__(self, name, modules, lr, clip=None, wd=None, wdpattern=r'.*'):
        ...
        self._opt = LossScaleOptimizer(self._opt, dynamic=True)
...
def _mnd_sample(self, sample_shape=(), seed=None, name='sample'):
    return tf.random.normal(tuple(sample_shape) + tuple(self.event_shape), self.mean(), self.stddev(), self.dtype, seed, name)
...
def _cat_sample(self, sample_shape=(), seed=None, name='sample'):
    ...
    indices = tf.random.categorical(self.logits_parameter(), sample_shape[0] if sample_shape else 1, self.dtype, seed, name)
    if not sample_shape:
        indices = indices[..., 0]
    return indices
