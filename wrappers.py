...
class ProcGen:
    ...
    def __init__(self, name, action_repeat=1, size=(64, 64), num_levels=200, distribution_mode='easy', seed=1):
        import gym
        name = ''.join(word for word in name.split('_'))
        self._env = gym.make('procgen:procgen-{}-v0'.format(name), num_levels=num_levels,
                             distribution_mode=distribution_mode, start_level=seed, rand_seed=seed)
        self._action_repeat = action_repeat
        self._size = size
        self._lives = None
        self._shape = self._env.observation_space.shape
